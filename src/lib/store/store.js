...
export const mappedCSVData = derived([csvData, distanceUnit], ([$csvData, $distanceUnit]) => {
  ...
  if ($csvData) {
    ...
    data = data.filter(
      (workout) =>
        typeof workout[TIMESTAMP_COLUMN] === "string" && workout[TIMESTAMP_COLUMN].length > 0
    );
...
export const classesTakenByInstructor = derived(activeData, ($activeData) => {
  return getClassesTakenByInstructor($activeData);
});

export const classesTakenByType = derived(activeData, ($activeData) => {
  if (!$activeData) {
    return [];
  }
